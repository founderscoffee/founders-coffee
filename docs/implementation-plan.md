...
| Event reminders        | DO alarms → Notifications Queue; low-frequency Cron recovery | Implemented by CO-02 on 2026-09-10, not deployed. `NotificationScheduleDO` holds one alarm per event, publishes `notification_due` to the queue and rearms from D1; the cron is a fifteen-minute recovery sweep. `apps/ui` binds the class across scripts, so worker-jobs deploys first                        |
...
| SMS fallback           | Twilio Programmable SMS via `libs/notifications`             | Implemented by CO-02, not deployed. One row per notification on push with `fallback_channel = 'sms'`; the parallel SMS/email scheduling is gone. The fallback is written only where the member consented, and no surface sets that consent until PF-08, so the SMS path is dark in the interim by design       |
...
| P0-018 | Blocked  | Jobs Worker                                               | AR-02 made the sweep resolve every row it selects, so delivery no longer stalls silently. ~~No queue is bound in any wrangler config~~ — queues are provisioned per environment and their consumers bound with a DLQ as of 2026-09-04, so the handler is reachable. Still blocked: **nothing produces into them**, so reminder polling is unchanged |
...
| P1-006 | Partial  | Event creation wizard and Mapbox venue selection                     | EC-01 through EC-10 have recorded code/staging completion. Production DNS/WAF and v0.1.0 release evidence are recorded for 2026-09-04; the authorized production creation smoke remains outstanding. See dated EC/deployment evidence, not the superseded staging-mailbox blocker.                                                                                                                                  |
| P1-007 | Partial  | Event feed/detail, virtualization, SEO metadata                      | Canonical/OG URL inheritance, missing sitemap, cookie-only locale indexing, incomplete event/city metadata and structured data, crawlable utility routes, missing social images, and unverified TanStack Start prerender configuration are tracked in the [SEO Implementation Plan](./seo-implementation-plan.md) as SEO-01 through SEO-12. GEO-01 through GEO-05 add the follow-up AI-readable content and structured-data parity work. Full prerender verification remains under P1-020 |
...
| P1-021 | Partial  | End-to-end tests                                                     | EC-09/10 recorded 18/18 locally and on staging across ar/fr/en at 390/768/1280 on 2026-09-03. Authorized production creation smoke and CO-11 remain; E2E stays outside CI.                                                                                                                                                                                                                                          |
...
| P1-024 | Planned  | SEO discoverability and search-engine operations                     | Implement SEO-01 through SEO-12 and the planned GEO-01 through GEO-05 follow-up in the [SEO Implementation Plan](./seo-implementation-plan.md): environment-aware indexation, canonical URLs, locale alternates, sitemap/robots, metadata, structured data, social previews, crawlable pagination, performance verification, structured-data parity, optional AI-readable surfaces, regression tests, and Search Console operations |
...
report measurement before enforcement in that dated snapshot. The later 2026-09-04
[deployment evidence](./deployment-evidence.md) supersedes it: CSP enforcement, Queue consumers,
the EC release and the Round Table redesign were recorded. This is documentary evidence, not a
new live CI or infrastructure certification.
...
1. **Plan 1 — EC-10 final handoff:** the
   ...
   v0.1.0 and DNS/WAF verification followed on 2026-09-04. One authorized production creation smoke
   remains outstanding before final operational handoff. Do not repeat the superseded staging
   mailbox or production DNS blockers, or imply the remaining smoke ran without authorization.
...
5. **CO-02 / P0-018 / P1-009:** done locally on 2026-09-10 and awaiting deployment. Per-event
   Durable Object alarms feed the environment-specific Notifications Queue, the cron is a
   fifteen-minute recovery sweep, delivery is push-first with SMS strictly behind it, and member
   preferences are enforced at send time. Consumers and DLQs already had dated deployment evidence;
   the producer binding and the Durable Object namespace are created by the next worker-jobs deploy.
