...
| 🟡 **P1-017** (Phase A) | **Phase A ✅** — app-wiring linchpin, see [`docs/app-wiring.md`](./app-wiring.md). `apps/ui/src/start.ts` (`createStart`, auto-discovered by the vite plugin — virtual server-entry `main` unchanged): `createCsrfMiddleware()` re-installed explicitly (defining `start.ts` disables the auto-CSRF) + global `requestContextMiddleware`. Env-injection solved: `getDb()` reads `cloudflare:workers` `env.DB` (the lib has no runtime wrangler → single-site cast). The markets `createServerFn` RPC wrappers (`getMarket`/`getMarketCities`/`getVisibleMarkets`, `strict:false` — `Market.brandOverrides` JSON col not provably serializable; zod validates inputs) call the db-injected resolver via `handleResult`. `apps/ui` `DB` binding (placeholder id → **P0-019**). `__root` beforeLoad detects locale → `<html lang dir>` (default `ar`/`rtl`); messages thread `{locale}` explicitly (no global — sidesteps #6268). Verified end-to-end in dev (a loader calling `getVisibleMarkets` reached Miniflare D1). **#6223 streaming-SSR: obsolete** — fixed in react-start ≥1.143.8 (installed 1.168.26); no action. **Phase B pending** — `authMiddleware`/`requirePermission` middleware instances. **Phase C pending** — observability app wiring (`/client-logs`, `configureClientLogger`, `reportError` in error boundaries) + CSRF prod-origin config | P0-009, P0-012, P0-014 | FR-L1..L6, NFR-7, NFR-8 | L |
