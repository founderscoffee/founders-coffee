...
| ✅ **P1-003** | Auth UI (email-OTP + OAuth), see [`docs/auth-ui.md`](./auth-ui.md). **Passwordless email-OTP + OAuth — no phone** (FR-A4/D4 supersede phone-OTP). `/login` two-step flow: email → inline **Turnstile widget** (no `@marsidev` dep; loads the CF script, token sent as `cf-turnstile-response`) → `send-verification-otp` → 6-digit code → `signIn.emailOtp` → session cookie → redirect to `?redirect=` or `/`. First sign-in auto-creates the user (no separate signup). OAuth buttons config-gated (`getPublicAuthConfig` server-fn exposes sitekey + hasSocial only). Navbar Login↔Logout via `better-auth/react` `useSession` **gated behind a mount flag** (better-auth's react-store trips the SSR "Invalid hook call" under react-dom/server). **Real OTP email wired** (`createAuthHandler(env, deps?)` → `createOtpEmailProvider` adapter bridging auth's `sendOtp` to `libs/email`'s `CloudflareEmailProvider` + reusing `NotificationEmail`; dev keeps `DevEmailProvider`, prod uses the `EMAIL` binding). Plain React state + Zod (no TanStack Form). Secure cookies verified on localhost. **Dev smoke (API)**: send-otp `{success}` + DevEmailProvider logs the code → sign-in sets `better-auth.session_token` → `get-session` returns the session. **Deferred**: dashboard auth (P1-006, shared cookie-domain session), SSR-correct session, localized OTP email, onboarding (P1-004) | P0-008, P0-010 | FR-A4, NFR-4 | L |
