...
| Identity rate limiting | Durable Object + WAF; Better Auth may additionally use D1    | Partial; `RateLimiterDO` is bound and deployed to staging (verified 2026-09-02), but account-side WAF rule IDs and staging behavior are unverified                                                                                                                                                             |
...
| P1-018 | Partial  | Security hardening                                                   | Event creation has DO + Turnstile + fail-closed WAF contract; RSVP is rate limited and permission-checked. Remaining, each with a ticket: account WAF evidence, no CSP or security headers anywhere (AR-08), Turnstile on RSVP, three anonymous unmetered map endpoints (AR-05), and three state-changing endpoints with no declared permission (AR-06) |
