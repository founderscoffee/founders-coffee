...
| **P0-013** | Zod validation convention — shared schema patterns + automatic server-fn input validation + inferred types exported to apps | P0-012 | NFR-4 | S |
...
| **P1-017** | i18n app wiring + parity: `__root` cookie-locale detection (`<html lang dir>`), localize every P1 screen in `ar` (RTL) with fallback; **disable TanStack Start streaming SSR** (the [#6223](https://github.com/TanStack/router/issues/6223) Arabic UTF-8 fix — verify the Start knob or `React.lazy` localized routes). **+ observability app wiring**: client `/client-logs` ingestion endpoint (→ `ingestClientLogs`), `configureClientLogger` bootstrap, `reportError` into `createStart` `onError`/route error boundaries | P0-009, P0-014 | FR-L1..L6, NFR-7, NFR-8 | M |
