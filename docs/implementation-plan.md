...
| 🔄 **P0-019** | Provision Cloudflare resources — **runbook written, execution pending account credentials**. See [`docs/provisioning.md`](provisioning.md). Scope reduced from the original plan: only **D1 + Vectorize** are provisioned per environment (`-staging` / `-production` suffixes), because no app declares R2, KV, Queues or Analytics Engine bindings — notifications flow through the D1 `scheduled_notifications` table + cron, not Queues. Email Sending (SPF/DKIM/DMARC) and Access remain manual dashboard steps | P0-011 | §4 | M |
| ✅ **P0-020** | CI/CD — **GitHub Actions**, not GitLab (the repo moved to GitHub). `ci.yml` (reusable): `nx sync:check` + `nx run-many -t typecheck lint test`, covering the Nx boundary rules and the Miniflare integration tests; Nx cache restored via `actions/cache`. `deploy.yml`: push to `develop`→staging automatically; production requires an **explicit manual run from `main`** (the `resolve` job refuses production from any other ref). A required-reviewer rule on the `production` GitHub Environment was the intended gate but needs GitHub Pro/Team on a private repo — `docs/ci.md` records the one-command switch once the plan allows it. Applies D1 migrations **before** any build, then deploys all four Workers. **Two Cloudflare environments** (`env.staging`/`env.production`) added to all four `wrangler.jsonc` files with per-environment worker names, custom domains, vars and bindings. Nx targets are `deploy:staging`/`deploy:production` — the bare `deploy` target was removed so nothing can publish to the unsuffixed top-level config. Note the asymmetry: the three Vite apps select their environment at **build** time via `CLOUDFLARE_ENV` (the Cloudflare Vite plugin flattens the config, so `wrangler deploy --env` is invalid for them), while `worker-jobs` uses `--env`. See [`docs/ci.md`](ci.md). **Fixed in passing:** `EventLiveDO` was declared `new_classes` (key-value-backed), which Cloudflare no longer allows for new namespaces and the Free plan never supported → now `new_sqlite_classes`; the cross-script `script_name` was dropped from both DO bindings so each environment binds its own script; and `libs/ui` component files were renamed to PascalCase (AGENTS §5) to fix a casing mismatch that broke `tsc` and would have hard-failed on case-sensitive CI runners | P0-001 | NFR-11, NFR-12 | M |
