...
- **Durable Object alarm per challenge schedule**: set or updated at publish/edit; each alarm performs the due transition and enqueues notifications. Workflows are reserved for durable multi-step work after a transition. No D1 polling.
...
| **Workflows**         | Durable multi-step operations after a lifecycle event is triggered                                          |
| **Durable Objects**   | Per-challenge phase alarms, live leaderboard coordination, and team-formation locks where needed            |
| **Queues**            | Push-first/SMS-fallback notifications plus async AI moderation/embedding                                    |
...
| **Cloudflare Email**  | Explicit email delivery such as certificates and billing                                                    |
...
```
Challenge
  id, market_code, commercial_client_id?, sponsor_id?, title, slug, problem_statement, rules, code_of_conduct
  ...
  prize_pool (Money), order_id? (present only for a commercial client service fee)
...
| **HACK-002** | **Lifecycle scheduler**: per-challenge Durable Object alarms drive phase transitions and notification triggers; Workflows only for multi-step durable operations; cancel/pause handling       | HACK-001, P0-018   | §3.1, §4.9         | Durable Objects, Workflows, Queues   | L    |
...
| **HACK-018** | **Operations and sponsor surfaces**: management tables in `apps/admin`; commercial-client/sponsor views in `apps/dashboard`; sponsor attachment → [SP-014](./sponsorship-measurement-plan.md) | HACK-011, P0-010   | FR-H7, FR-M1/M2    | TanStack Table, D1                   | M    |
