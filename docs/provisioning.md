...
The command requires `rollback.yml` to be present on the repository default branch so the rollback
is executed by GitHub Actions. If that workflow has not reached the default branch yet, the command
fails before any local rollback. An operator may explicitly use
`npm run rollback:staging -- --local-fallback` for the pre-default-branch escape hatch; it performs
the same Worker-only rollback locally with each Worker's canonical Wrangler config, then continues
with the final deploy. A failed rollback stops before the final deploy so an operator can inspect
the mixed state and recover deliberately.
