...
| `events/rpc.ts`                        | `createEvent` (POST + authz + DO limit + WAF gate + Turnstile + Zod), `getEvent`, `getUpcomingEvents`                                                     | ✅ EC-05/06 code; WAF account evidence pending                 |
...
| `turnstile/*`                          | Siteverify provider, development runtime, deployed fail-closed WAF contract, and event-create middleware                                                  | ✅ EC-06 code; WAF account evidence pending                    |
