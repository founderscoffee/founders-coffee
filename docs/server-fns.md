...
profiles, markets, free events, RSVP, trust/moderation, notifications, and their operations. Turnstile
is rendered and verified only for anonymous/public operations; authenticated profile, account, photo,
feedback, RSVP, and operations mutations use the session, centralized permission checks, and rate
limits without a browser challenge. Public authentication and waitlist writes remain challenge-gated.
...
| `profile/rpc.ts`                       | Owner/public profile projections, profile/preferences mutations, photo upload/removal, and masked account read model; the legacy `setHomeLocation` endpoint is a typed refresh-required tombstone | ✅ PF-03a through PF-08; authenticated mutations do not require Turnstile                                                         |
...
| `events/rpc.ts`                        | `createEvent` (POST + authz + DO limit + WAF gate + Zod), `getEvent`, cursor-aware `getUpcomingEvents`, `getHostedEvents`                                                                         | ✅ EC-05/06; shared Free-plan WAF active · SEO-08; event-create Turnstile exception recorded under P1-018                         |
| `rsvps/*`                              | RSVP/cancellation RPCs, RSVP counters, and notification production                                                                                                                                | Atomic capacity and idempotency fixed by AR-04/CO-02; session-bound RSVP uses authz and rate limiting without a browser challenge |
...
| `turnstile/*`                          | Siteverify provider, development runtime, public waitlist challenge middleware, and the deployed event-create WAF contract                                                                        | ✅ EC-06; public operations only; authenticated profile/account/feedback mutations intentionally bypass the browser challenge     |
