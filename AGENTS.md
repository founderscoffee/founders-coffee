...
- **Files cap at 300 lines.** Enforced as a build-breaking lint error (`max-lines`), counting blank and comment lines so the number matches `wc -l`. A file over the cap is doing more than one job — **split it by responsibility**, never reformat it under the limit. The only exemptions are files another rule requires to be single (`libs/db/src/schema.ts` per §11, the versioned geo datasets); they are enumerated in `tools/eslint/file-globs.mjs`, never disabled inline. The cap binds the toolchain too: `eslint.config.mjs`, `vitest.workspace.ts`, and `tools/` are linted by the `workspace-root` project, so a root file cannot outgrow the rule it enforces.
...
- **Comments.** Names and structure are the documentation. Enforced as build-breaking lint errors (`local/comment-policy`, `local/no-comments`), and auto-fixable — `eslint --fix` deletes anything the policy disallows.
  ...
  - **Config files carry no comments at all** (`*.config.*`, `vitest.workspace.ts`, `eslint.config.mjs`), enforced by `local/no-comments`. A config file is wiring — which plugin, which glob, which path — and prose beside a setting drifts from the setting. Rationale for a lint rule lives beside the rule in `tools/eslint/`; rationale for a decision lives in `docs/` and the commit that made it.
  - **Other non-TypeScript files** (`.js`, `.mjs`, `.cjs`) keep the looser rule: no `//` line comments, block comments allowed. `tools/eslint/` is deliberately in this tier — a lint rule's semantics need explaining where they are implemented.
